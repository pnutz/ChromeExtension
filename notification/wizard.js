...
var Wizard = {
  ...
  init: function() {
    ...
    // store generated in global variables
    // lazy load them & html elements when user gets to it
    setTimeout(function() {
  ...
  setCurrencyByCode: function(code) {
    if (code !== "") {
      $(Wizard.configurations.formFields.currency.id + " option").each(function() {
        var selectText = $(this).text();
        var currencyCode = selectText.substring(0, selectText.indexOf(" "));
        if (code === currencyCode) {
          Wizard.setFieldValue("currency", $(this).val());
          return false;
        }
      });
    }
    ...
    switch(event.data.request) {
      ...
      case "generatedData":
        $.each(event.data.generated, function(key, value) {
          if (key !== "templates" && key !== "elementPaths" && key !== "items" && key !== "taxes" && key !== "currency") {
            Wizard.setFieldValue(key, value);
          } else if (key === "items") {
            if (Wizard.itemTable.addDataRow != null) {
            ...
            if (Wizard.taxTable.addDataRow != null) {
            ...
            if (Wizard.configurations.formFields[key] != null) {
              // if currency select list is already populated
              if ($(Wizard.configurations.formFields.currency.id + " option").length > 0) {
                Wizard.setCurrencyByCode(value);
              } else {
                WebAppObserver.addObserver(key, value);
              }
            }
      ...
      case "searchResults":
        ...
        if (event.data.itemIndex == null) {
          Wizard.setAutoCompleteOptions(event.data.fieldName, event.data.results);
        }
        // handsontable
        else {
          if (event.data.tableType === "item") {
            ...
            if (!Wizard.itemTable.source.hasOwnProperty(event.data.itemIndex)) {
              Wizard.itemTable.source[event.data.itemIndex] = {};
            }
            Wizard.itemTable.source[event.data.itemIndex][event.data.fieldName] = event.data.results;

            // update table autocomplete source and open TwoReceiptEditor
            Wizard.itemTable.updateTableSource(event.data.fieldName, event.data.itemIndex);
          } else {
            // store search results in source so handsontable can switch between sources for different cells
            if (!Wizard.taxTable.source.hasOwnProperty(event.data.itemIndex)) {
              Wizard.taxTable.source[event.data.itemIndex] = {};
            }
            Wizard.taxTable.source[event.data.itemIndex][event.data.fieldName] = event.data.results;

            // update table autocomplete source and open TwoReceiptEditor
            Wizard.taxTable.updateTableSource(event.data.fieldName, event.data.itemIndex);
          }
        }
        break;
      ...
      case "newItemRows":
        for (var i = 0; i < event.data.items.length; i++) {
          Wizard.itemTable.addDataRow(event.data.items[i]);
        }
        break;

      case "getRowData":
        var row = Wizard.itemTable.getDataAtRow(event.data.itemIndex);
